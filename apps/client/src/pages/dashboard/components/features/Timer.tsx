...
const Timer = ({ onComplete }: TimerProps) => {
  ...
  const getTimerText = () => {
    ...
    if (error) return '시간 로드 실패';
    if (timerState.isExpired) return '타이머 완료';
    return formattedTime;
  };

  const getTimerColor = () => {
    if (isLoading) return 'text-gray400';
    if (error) return 'text-error400';
    if (timerState.isExpired) return 'text-gray400';
    return 'text-gray400';
  };
